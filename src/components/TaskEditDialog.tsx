  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-md bg-background border-border">
        <DialogHeader>
          <DialogTitle className="text-lg font-semibold tracking-[-0.02em]">Edit task</DialogTitle>
        </DialogHeader>
        <div className="space-y-4 pt-2">
          ...
          <div>
            ...
            <div className="relative mt-1.5">
              ...
              <Input
                ...
                className={cn(
                  "h-10 bg-secondary border-0 text-[13px] pl-9",
                  location.trim() && /\d/.test(location) && "ring-1 ring-accent/30"
                )}
                placeholder="e.g. Office, 123 Main St…"
              />
              {location.trim() && /\d/.test(location) && (
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[9px] text-accent font-medium">
                  📍 Address
                </span>
              )}
            </div>
          ...
          <div>
            ...
            <Textarea value={description} onChange={(e) => setDescription(e.target.value)} className="mt-1.5 bg-secondary border-0 text-[13px] min-h-[60px]" />
          </div>
